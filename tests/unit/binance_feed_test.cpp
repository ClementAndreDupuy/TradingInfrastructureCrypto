...
TEST_F(BinanceFeedHandlerTest, HandlerCreationWithApiKey) {
    auto handler_with_key = std::make_unique<BinanceFeedHandler>("BTCUSDT", "test_api_key", "test_api_secret");
...
TEST_F(BinanceFeedHandlerTest, SequenceValidation) {
    ...
    std::string msg = R"({"e":"depthUpdate","s":"BTCUSDT","U":)" +
        std::to_string(initial_seq + 100) + R"(,"u":)" + std::to_string(initial_seq + 100) +
        R"(,"b":[],"a":[]})";
...
TEST_F(BinanceFeedHandlerTest, BinanceSequenceRule) {
    ...
    std::string msg1 = R"({"e":"depthUpdate","s":"BTCUSDT","U":)" +
        std::to_string(last_id + 1) + R"(,"u":)" + std::to_string(last_id + 1) + R"(,"b":[],"a":[]})";
    EXPECT_EQ(handler_->process_message(msg1), Result::SUCCESS);

    last_id = handler_->get_sequence();

    std::string msg2 = R"({"e":"depthUpdate","s":"BTCUSDT","U":)" +
        std::to_string(last_id) + R"(,"u":)" + std::to_string(last_id + 2) + R"(,"b":[],"a":[]})";
