...
class AlphaMetrics:
    ...
    def __str__(self) -> str:
        return (
            f"IC={self.ic_mean:.4f}  ICIR={self.icir:.4f}  "
            f"HitRate={self.hit_rate:.3f}  α={self.ols_alpha:.6f} "
            f"(t={self.ols_t_stat:.2f} p={self.ols_p_value:.4f})  "
            f"β={self.ols_beta:.4f}  R²={self.r_squared:.4f}  "
            f"Turnover={self.turnover:.6f}  N={self.n_samples}"
        )


def compute_ic(signals: np.ndarray, returns: np.ndarray,
               rolling_window: int = 20) -> tuple[float, float, float]:
    """
    ...
    if not ics:
        ...
        return float(rho) if np.isfinite(rho) else 0.0, 1e-9, 0.0

    ic_arr  = np.array(ics)
    ic_mean = float(ic_arr.mean())
    ic_std  = float(ic_arr.std(ddof=1)) if len(ic_arr) > 1 else 1e-9
...
def ols_regression(signals: np.ndarray, returns: np.ndarray
    ...
    # t-stat for alpha (intercept)
    n = len(s)
    x_mean  = s.mean()
    ss_xx   = ((s - x_mean) ** 2).sum()
    residuals = r - (intercept + slope * s)
    s2      = (residuals ** 2).sum() / (n - 2 + 1e-9)
    se_int  = np.sqrt(s2 * (1.0 / n + x_mean ** 2 / (ss_xx + 1e-9)))
    t_stat  = intercept / (se_int + 1e-9)
...
def print_alpha_report(metrics: AlphaMetrics, backtest_metrics: dict | None = None) -> None:
    ...
    print("=" * 65)
    print(f"  IC (mean / std / ICIR) : {metrics.ic_mean:+.4f} / {metrics.ic_std:.4f} / {metrics.icir:+.4f}")
