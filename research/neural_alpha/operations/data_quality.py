...
class DataQualityConfig:
    ...
    max_gap_ns: int = 5 * 60 * 1000000000
    max_age_ns: int = 26 * 3600 * 1000000000
    allow_future_ticks: bool = False
    future_tolerance_ns: int = 60 * 1000000000
...
class DataQualityError(RuntimeError):
    ...
    def __init__(self, report: QualityReport) -> None:
        ...
        super().__init__(
            f"Data quality gates failed: {breaches}. Inspect the quality report for details."
        )
...
def _check_sequence_gaps(df: pl.DataFrame, max_gap_ns: int) -> CheckResult:
    ...
    reason = f"venues with gap > {max_gap_ns}ns: {breaching_venues}" if not passed else ""
    return CheckResult(
        check="sequence_gap_check",
        passed=passed,
        breach_reason=reason,
        affected_venues=breaching_venues,
        details={"max_gap_ns_threshold": max_gap_ns, "max_gap_ns_observed_per_venue": max_observed},
    )
...
def _check_timestamp_skew(
    ...
    if too_old:
        reason_parts.append(
            f"oldest tick is {oldest_age_ns / 1000000000.0:.1f}s old (limit {max_age_ns / 1000000000.0:.1f}s)"
        )
    if too_future:
        reason_parts.append(
            f"newest tick is {future_skew_ns / 1000000000.0:.1f}s in the future (tolerance {future_tolerance_ns / 1000000000.0:.1f}s)"
        )
    return CheckResult(
        check="timestamp_skew",
        passed=passed,
...
def _check_duplicates(df: pl.DataFrame) -> CheckResult:
    ...
    if n_dupes > 0 and "exchange" in df.columns:
        ...
        affected_venues = (
            df.filter(dup_mask)["exchange"].unique().to_list() if "exchange" in df.columns else []
        )
    passed = n_dupes == 0
...
def run_quality_gates(df: pl.DataFrame, cfg: DataQualityConfig | None = None) -> QualityReport:
    ...
    checks: list[CheckResult] = [
        ...
        _check_timestamp_skew(df, cfg.max_age_ns, cfg.allow_future_ticks, cfg.future_tolerance_ns),
