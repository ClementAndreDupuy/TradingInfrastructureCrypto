...
class BacktestConfig:
    ...
    adv_usd: float = 5000000.0
    impact_coeff: float = 0.2
    impact_square_root: bool = False
    queue_decay_lambda: float = 1.5
...
class NeuralAlphaBacktest:
    ...
    def run(self, df: pl.DataFrame, signals: np.ndarray) -> dict:
        ...
        exit_thresh = self.cfg.exit_threshold_bps * 0.0001
        stop_loss = self.cfg.stop_loss_bps * 0.0001
        lat = self.cfg.latency_ticks
        fee_rate = self.cfg.taker_fee_bps * 0.0001
        ...
        for t in range(T):
            ...
            if self._position != 0.0:
                ...
                if current_val / (abs(self._position) * self._entry_price + 1e-09) < -stop_loss:
    ...
    def _open_position(
        self, t: int, fill_px: float, qty: float, side: str, ts: int, fee_rate: float
    ) -> None:
    ...
    def _close_position(
        self, t: int, bid: float, ask: float, ts: int, fee_rate: float, reason: str
    ) -> None:
    ...
    def _apply_market_impact(self, fill_px: float, qty: float) -> float:
        ...
        adv = max(self.cfg.adv_usd, 1e-09)
    ...
    def _queue_fill_probability(self, level_price: float, qty: float) -> float:
        queue_depth = max(level_price * qty, 1e-09)
    ...
    def _compute_metrics(self, signals: np.ndarray) -> dict:
        ...
        return {
            ...
            "profit_factor": (
                float(np.sum(wins) / abs(np.sum(losses))) if losses.size else float("inf")
            ),
    ...
    def _compute_time_aware_sharpe(self) -> float:
        ...
        if np.any(capital_curve <= 0):
            return 0.0
        dt_s = np.diff(ts_ns.astype(np.float64)) / 1000000000.0
        valid = dt_s > 0
        if not np.any(valid):
            return 0.0
