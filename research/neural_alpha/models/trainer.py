...
def pretrain_spatial(
    ...
    if _first_batch is not None:
        ...
        if float(_lob.std(dim=1).mean()) < 0.0001:
            print(
                f"  [pretrain] skipped — LOB near-constant across time (temporal std={float(_lob.std(dim=1).mean()):.2e})"
            )
            return
    for epoch in range(epochs):
        ...
        for batch in loader:
            ...
            lob_std = lob.std(dim=(0, 1), keepdim=True).clamp(min=1e-06)
            ...
            labels = torch.cat(
                [torch.arange(B, 2 * B, device=device), torch.arange(0, B, device=device)]
            )
...
def walk_forward_train(df, cfg: TrainerConfig | None = None) -> list[dict]:
    ...
    for fold_idx, (train_df, test_df) in enumerate(splits):
        ...
        if cfg.fold_seed_offset > 0:
            ...
            np.random.seed(cfg.fold_seed_offset + fold_idx * 17 & 4294967295)
        ...
        if cfg.pretrain and cfg.pretrain_epochs > 0:
            ...
            pretrain_spatial(
                model, train_loader, cfg.pretrain_epochs, cfg.lr, device, cfg.adv_noise_std
            )
            ...
            if should_log:
                ...
                if val_loss < best_val_loss:
                    ...
                    if cfg.early_stop_patience > 0 and epochs_no_improve >= cfg.early_stop_patience:
                        print(
                            f"  Early stopping at epoch {epoch + 1} (no improvement for {epochs_no_improve} epochs)"
                        )
                        break
