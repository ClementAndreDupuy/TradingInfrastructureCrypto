...
**Work items**
- [x] Add shadow execution attribution for decision price, fill price, edge at entry, implementation shortfall, hold time, and venue-level markout.
- [x] Log parent intent metadata: signal, risk, target position, urgency, expected cost, and reason for entry/exit/hold.
- [x] Extend shadow session summary with fee drag, spread paid/captured, average inventory age, fill-to-markout, and per-venue contribution.
- [x] Produce one comparison report from an existing shadow run that explains whether losses came from fees, slippage, churn, or stale inventory.

**Acceptance criteria**
- [x] A shadow run emits enough metrics to explain a negative net PnL without reading raw order logs manually.
- [x] The end-of-run summary includes per-venue execution quality, not only fill counts and net PnL.
- [x] At least one replay or shadow report attributes losses into fees, slippage, and adverse-selection buckets.
...
**Work items**
- [x] Add a `PositionLedger` for symbol-level and venue-level inventory, average entry price, realized/unrealized PnL, pending orders, and inventory age.
- [x] Feed the ledger from shadow/live fills and top-of-book mid-price updates.
- [x] Expose a stable snapshot interface consumable by the engine loop, router, and shadow summary.
- [x] Add tests covering partial fills, partial exits, cross-venue inventory, and inventory aging.

**Acceptance criteria**
- [x] Ledger global position matches aggregate connector position across all venues within tolerance.
- [x] Venue-level inventory survives partial fills/exits and never requires inference from logs.
- [x] Tests cover long entry, partial unwind, full flatten, and venue divergence cases.
...
**Work items**
- [x] Add a `PortfolioIntentEngine` that consumes alpha, regime, current portfolio state, venue health, and expected costs.
- [x] Compute `target_global_position`, `position_delta`, `urgency`, `flatten_now`, and `max_shortfall_bps`.
- [x] Support long-only mode first; short-enabled logic can remain feature-flagged.
- [x] Log why each intent changed: stronger alpha, alpha decay, negative reversal, illiquid regime, stale inventory, etc.

**Acceptance criteria**
- [x] No direct entry/exit orders are created from raw signal thresholds alone in the new path.
- [x] Identical signal inputs produce deterministic target-position outputs in unit tests.
- [x] The engine can explain every state change via machine-readable reason codes.
...
**Work items**
- [x] Add a `ParentOrderManager` that owns a working plan: side, total qty, remaining qty, deadline, urgency, and execution style permissions.
- [x] Connect plan progress to fills, cancels, rejects, and expiry.
- [x] Support plan replacement when target position changes meaningfully.
- [x] Add tests for fill progression, cancellation, replacement, and deadline expiry.

**Acceptance criteria**
- [x] A target-position change creates or updates one parent execution plan, not ad hoc child orders every loop.
- [x] Remaining quantity and plan state are always observable.
- [x] Plans expire or escalate when their deadline passes.
...
**Work items**
- [x] Add a `ChildOrderScheduler` that selects between passive join, passive improve, marketable IOC, and sweep.
- [x] Refactor `SmartOrderRouter` into a lower-level scorer used by the scheduler.
- [x] Include venue quality, queue risk, fill probability, toxicity, latency, and inventory-age penalties.
- [x] Make the scheduler horizon-aware so short-lived alpha trades more aggressively than long-lived alpha.

**Acceptance criteria**
- [x] Execution style depends on urgency and horizon rather than a fixed IOC-style submit path.
- [x] Venue selection is based on expected shortfall, not just displayed best price.
- [x] Unit tests verify that high-toxicity or low-fill venues are deprioritized.
