...
- [x] **NA-R-3: Stabilize neural-alpha continuous retrain under illiquid windows**
  ...
  - Acceptance criteria:
    ...
    - [x] HPO logging includes per-component validation losses (`loss_return`, `loss_direction`, `loss_risk`) to localize explosion source.
    - [x] Continuous retrain is skipped (or downgraded) when order-count columns are all-zero for the window.
