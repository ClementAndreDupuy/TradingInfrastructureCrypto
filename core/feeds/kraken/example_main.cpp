...
int main() {
    ...
    // Kraken uses pair names like "XBTUSD" (BTC/USD) or "ETHUSD"
    ...
    handler.set_snapshot_callback([](const Snapshot& snapshot) {
        ...
        std::cout << "Exchange: " << exchange_to_string(snapshot.exchange) << std::endl;
    ...
    handler.set_delta_callback([](const Delta& delta) {
        std::cout << "Delta: " << side_to_string(delta.side) << " "
                  << delta.price << " @ " << delta.size
                  << " (seq=" << delta.sequence << ")" << std::endl;
    });
    ...
    std::cout << "\nReal-time data from Kraken API" << std::endl;
