...
public:
    ...
    explicit OrderManager(ExchangeConnector& connector)
        : connector_(connector), next_id_(1) {
    ...
    ConnectorResult cancel(uint64_t client_id) {
        return connector_.cancel_order(client_id);
    }

    ConnectorResult cancel_all(const char* symbol) {
        return connector_.cancel_all(symbol);
    }
...
private:
    ...
    ManagedOrder* find_free_slot() {
        for (auto& s : slots_) if (!s.active) return &s;
        return nullptr;
    }

    ManagedOrder* find_order(uint64_t client_id) {
        for (auto& s : slots_) {
            if (s.active && s.order.client_order_id == client_id) return &s;
        }
        return nullptr;
    }
