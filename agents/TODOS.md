...
- [ ] **M5** `core/feeds/common/book_manager.hpp` — Add `set_publisher(LobPublisher*)` and a private `publish_lob(int64_t timestamp_ns)` helper. Call `publish_lob` after every successful `apply_snapshot` and `apply_delta`. Pre-allocate `pub_bids_` / `pub_asks_` vectors (capacity 5) as class members to avoid hot-path heap allocation. Publisher pointer is optional; skip silently when null or not open.
