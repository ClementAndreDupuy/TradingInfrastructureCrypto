...
- **`core/engine/trading_engine_main.cpp`** + **`docs/TODOS_EXECUTION_ENGINE.md`** — Live execution now reads venue reconciliation snapshots, feeds the same target-position loop used in shadow with actual live inventory, and the roadmap no longer assumes a canary rollout or legacy live fallback path.
